// Aurora WebGL background — converted to ES module for Vite
// Uses ogl via esm.sh CDN (no npm install needed)

let oglLoaded = false;
let oglModule = null;

async function loadOgl() {
  if (oglLoaded) return oglModule;
  try {
    oglModule = await import('https://esm.sh/ogl@1.0.11');
    oglLoaded = true;
    return oglModule;
  } catch (e) {
    console.warn('Aurora: ogl failed to load', e);
    return null;
  }
}
...
export async function mountAurora(ctn, props = {}) {
  ...
  const state = {
    ...
    amplitude:  props.amplitude  ?? 0.9,
    blend:      props.blend      ?? 0.5,
    speed:      props.speed      ?? 0.45,
  };
  ...
  gl.canvas.style.cssText = 'background:transparent;display:block;';

  let program;

  function resize() {
    const w = ctn.offsetWidth || window.innerWidth;
    const h = ctn.offsetHeight || 400;
  ...
  program = new Program(gl, {
    ...
    uniforms: {
      ...
      uResolution: { value: [ctn.offsetWidth || window.innerWidth, ctn.offsetHeight || 400] },
